...
class spr55demosmViewSet(BaseConnectorViewSet):
    PASSWORD = settings.CONNECTOR_SPR55DEMOSM_PASSWORD
    USERNAME = settings.CONNECTOR_SPR55DEMOSM_USERNAME
    BASE_URL = "https://api.demosm.com"
    AUTH_TYPE = "basic"
    IDENTIFIER = "CONNECTOR_SPR55DEMOSM"

    @action(detail=False, methods=["get"], url_path="search/testing")
    def search_testing(self, request, *args, **kwargs):
        params = request.query_params
        payload = request.data
        data = self._api_call(
            "/search/testing", request_type="get", payload=payload, params=params
        )
